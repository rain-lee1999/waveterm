...
| term:buttonbar <VersionBadge version="v0.15" />                      | object[] | shows a SecureCRT-style button bar at the bottom of terminal blocks. Each button supports `label`, `command`, optional `tooltip`, and optional `addnewline` (defaults to false)                                                                                |
